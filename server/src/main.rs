...
#[derive(Debug, Serialize, Deserialize)]
pub struct AuthResp { pub token: String, pub user: UserPublic }
#[derive(Debug, Serialize, Deserialize)]
pub struct UserPublic { pub email: String, pub display_name: Option<String>, pub role: String, pub is_verified: bool }

async fn health_check() -> impl IntoResponse {
    (StatusCode::OK, Json(serde_json::json!({"status":"ok","service":"w9-db","timestamp":Utc::now().to_rfc3339()})))
}
    ...
    if let Some(user) = users.get(&req.email) {
        if user.password_hash == req.password {
            ...
            return (StatusCode::OK, axum::Json(serde_json::json!({
                "token": token, "user": {"email": user.email, "display_name": user.display_name, "role": user.role, "is_verified": user.is_verified}
            })));
        }
    }
...
async fn handle_register(
...
) -> (StatusCode, axum::Json<serde_json::Value>) {
    {
        ...
        if users.contains_key(&req.email) {
            return (StatusCode::CONFLICT, axum::Json(serde_json::json!({"error":"Email exists"})));
        }
    }
    ...
    (StatusCode::CREATED, axum::Json(serde_json::json!({
        "token": token, "user": {"email": req.email, "display_name": req.display_name, "role": "user", "is_verified": true}
    })))
}
        ...
        if let Some(token) = auth.strip_prefix("Bearer ") {
            if let Some(email) = token.strip_prefix("jwt-").and_then(|t| t.split('-').next()) {
                if let Some(user) = state.users.read().unwrap().get(email) {
                    return (StatusCode::OK, axum::Json(serde_json::json!({
                        "email": user.email, "display_name": user.display_name, "role": user.role, "is_verified": user.is_verified
                    })));
                }
            }
        }
...
async fn oidc_discovery() -> impl IntoResponse {
    ...
    (StatusCode::OK, Json(serde_json::json!({
        "issuer": issuer, "authorization_endpoint": format!("{}/authorize", issuer),
        "token_endpoint": format!("{}/oauth/token", issuer), "userinfo_endpoint": format!("{}/userinfo", issuer),
        "response_types_supported": ["code"], "scopes_supported": ["openid","profile","email"],
    })))
}
